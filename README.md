...
* **2D map** — вид сверху: occupancy grid (чёрное — препятствия, светлое — свободно,
  серое — неизвестно) и граф поз с рёбрами соседства и loop closure; красно-зелёные оси —
  текущее положение камеры. Локальные сетки считает сам RTAB-Map (`RGBD/CreateOccupancyGrid`),
  проект только собирает их в глобальную карту и показывает.
