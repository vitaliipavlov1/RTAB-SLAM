...
```text
src/
├── main.cpp                  разбор аргументов, главный цикл, завершение
├── realsense_capture.{h,cpp} камера: свежий RGB-D кадр + ориентация от IMU
├── rtabmap_app.{h,cpp}       RTAB-Map: параметры, база, одометрия, карта
├── viewer.{h,cpp}            три окна и HUD
└── map_export.{h,cpp}        сохранение карты: облако PCL, фильтр, статистика
```

Модули независимы: каждый принимает свою маленькую конфигурацию (`CaptureConfig`,
`AppConfig`, `ViewerConfig`, `ExportPaths`), общих глобальных структур между ними нет.
`main.cpp` знает про все четыре, а они друг о друге — нет.
